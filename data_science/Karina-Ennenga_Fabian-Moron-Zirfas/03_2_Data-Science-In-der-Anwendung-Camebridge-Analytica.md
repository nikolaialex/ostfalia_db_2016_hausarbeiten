...
Als nächstes werfen wir einen Blick auf unsere `reduce` Funktion. Hier muss jedes einzelne Objekt inspiziert werden und mit allen anderen Elementen im Array verglichen werden. Ist es noch nicht in der Ergebnismenge enthalten wird ein neuer Eintrag erstellt. Ist es bereits enthalten wird der entsprechende Zähler inkrementiert. In diesem speziellem Fall nutzen wir ein temporäres Objekt um wiederkehrende Einträge vergleichen zu können. Die zweite Schleife `for(let key in obj)…` ist nur dazu da um die Daten wieder in das gleiche Format aufzubereiten in dem sie auch eingetroffen sind.     
